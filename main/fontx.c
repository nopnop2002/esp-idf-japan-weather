...
void InitFontx(FontxFile *fxs, const char *f0, const char *f1)
{
	AddFontx(&fxs[0], f0);
	AddFontx(&fxs[1], f1);
	if (strlen(f0)) OpenFontx(&fxs[0]);
	if (strlen(f1)) OpenFontx(&fxs[1]);
}
...
bool GetFontx(FontxFile *fxs, uint16_t sjis, uint8_t *pGlyph, uint8_t *pw, uint8_t *ph)
{
  
	int i;
	uint32_t offset;

	if(FontxDebug) printf("[GetFontx]sjis=0x%x\n",sjis);
	for(i=0; i<2; i++){
	//for(i=0; i<1; i++){
		...
		if(sjis < 0x100){
			...
			if(FontxDebug) printf("[GetFontx]fxs.is_ank fxs.fsz=%d\n",fxs[i].fsz);
				...
				if(fseek(fxs[i].file, offset, SEEK_SET)) {
					printf("Fontx:fseek(%"PRIu32") failed.\n",offset);
					return false;
				}
...
static FILE *fp_sjis; // UTF8-->SJIS Conversion file

int initSJIS(char * table) {
	fp_sjis = fopen(table, "r");	
	if (fp_sjis == NULL) {
		printf("initSJIS:%s not found.\n",table);
		return 0;
	}
	return 1;
}
...
uint16_t UTF2SJIS(uint8_t *utf8) {
	if (fp_sjis == NULL) {
		printf("UTF2SJIS:fp_sjis not open.\n");
		return 0;
	}
	...
	if(FontxDebug) printf("[UTF2SJIS] offset=%"PRIu32"\n",offset);
	char buf[2];
	if(fseek(fp_sjis, offset, SEEK_SET)) {
		printf("UTF2SJIS:fseek(%"PRIu32") failed.\n",offset);
		return 0;
	}
	if(fread(buf, sizeof(buf), 1, fp_sjis) != 1) {
		printf("UTF2SJIS:fread(%"PRIu32") failed.\n",offset);
		return 0;
	}
